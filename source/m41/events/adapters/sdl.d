module m41.events.adapters.sdl;

import derelict.sdl2.sdl;
import m41.events.adapters.adapter;

immutable uint[Sint32] sdl_keymap = [
...
* TODO
*/
final class SDLEventAdapter : EventAdapter {

    /**
    * Initializes the Adapter, called in loading
    */
    void initialize() {
    	SDL_InitSubSystem(SDL_INPUT);
    }
    ...
    void update() {
        ...
        while(SDL_PollEvent(&event)) {
            switch(event.type) {
                case SDL_KEYDOWN:
                    auto key = sdl_keymap.get(event.key.keysym.sym, null);
                ...
                case SDL_MOUSEMOTION:
                    ...
                    this.mouseMotion(event.motion.x, event.motion.y, event.motion.xrel, event.motion.yrel);
                break;
                case SDL_QUIT:
                    this.quitRequest();
                break;
